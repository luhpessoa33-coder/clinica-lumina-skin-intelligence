...
export function runtimeConfigurationIssues() {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(ENV.ownerEmail)) issues.push("OWNER_EMAIL_FORMAT");
  try {
    const appUrl = new URL(ENV.appBaseUrl);
    if (appUrl.protocol !== "https:") issues.push("APP_BASE_URL_HTTPS_REQUIRED");
  } catch { issues.push("APP_BASE_URL_FORMAT"); }
  try {
